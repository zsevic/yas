...
/* 2 */
  /***/ function (module, exports, __webpack_require__) {
    ...
    fetch('https://vd2wzffzz8.execute-api.eu-central-1.amazonaws.com/latest').then(response => response.json()).then(res => {
      let schedule = []
      res.forEach(el => {
        if (!schedule[el.day]) {
          schedule[el.day] = []
        ...
        if (!schedule[index + 1]) {
          ...
          while (i < 13) {
            if (!schedule[index + 1][i + 1]) {
              ...
              let lecture = `<td colspan="${schedule[index + 1][i + 1].duration}" style="text-align:center;">
